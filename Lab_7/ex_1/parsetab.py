...
_lr_productions = [
  ...
  ('prog -> decl_list stmt_list','prog',2,'p_prog','myParser.py',106),
  ('decl_list -> decl_list decl','decl_list',2,'p_decl_list','myParser.py',128),
  ('decl_list -> <empty>','decl_list',0,'p_decl_list','myParser.py',129),
  ('decl -> type var_list S','decl',3,'p_decl','myParser.py',134),
  ('decl -> type error S','decl',3,'p_decl_error','myParser.py',140),
  ('decl -> type var_list error','decl',3,'p_decl_error','myParser.py',141),
  ('type -> INT_TYPE','type',1,'p_type','myParser.py',152),
  ('type -> DOUBLE_TYPE','type',1,'p_type','myParser.py',153),
  ('m_copy -> empty','m_copy',1,'p_m_copy','myParser.py',164),
  ('var_list -> var','var_list',1,'p_var_list','myParser.py',172),
  ('var_list -> var_list CM m_copy var','var_list',4,'p_var_list','myParser.py',173),
  ('var -> ID','var',1,'p_var','myParser.py',186),
  ('var -> ID SO INT SC','var',4,'p_var','myParser.py',187),
  ('stmt_list -> stmt_list stmt','stmt_list',2,'p_stmt_list','myParser.py',214),
  ('stmt_list -> stmt','stmt_list',1,'p_stmt_list','myParser.py',215),
  ('stmt_list -> error stmt','stmt_list',2,'p_stmt_list_error','myParser.py',221),
  ('stmt -> if','stmt',1,'p_stmt','myParser.py',228),
  ('stmt -> while','stmt',1,'p_stmt','myParser.py',229),
  ('stmt -> assignment','stmt',1,'p_stmt','myParser.py',230),
  ('stmt -> print','stmt',1,'p_stmt','myParser.py',231),
  ('stmt -> BO stmt_list BC','stmt',3,'p_stmt','myParser.py',232),
  ('stmt -> BO stmt_list error BC','stmt',4,'p_stmt_error','myParser.py',237),
  ('stmt -> BO error BC','stmt',3,'p_stmt_error','myParser.py',238),
  ('stmt -> error S','stmt',2,'p_stmt_error','myParser.py',239),
  ('assignment -> id S','assignment',2,'p_assignment','myParser.py',254),
  ('assignment -> id EQ exp S','assignment',4,'p_assignment','myParser.py',255),
  ('assignment -> id EQ error S','assignment',4,'p_assignment_error','myParser.py',269),
  ('assignment -> error EQ exp S','assignment',4,'p_assignment_error','myParser.py',270),
  ('assignment -> id EQ exp error','assignment',4,'p_assignment_error','myParser.py',271),
  ('print -> PRINT id S','print',3,'p_print','myParser.py',287),
  ('print -> PRINT error S','print',3,'p_print_error','myParser.py',297),
  ('if -> IF if_condition nt0_if stmt ELSE nt1_if stmt','if',7,'p_if','myParser.py',305),
  ('if -> IF if_condition nt0_if stmt','if',4,'p_if','myParser.py',306),
  ('if -> IF if_condition nt0_if stmt error nt1_if stmt','if',7,'p_if','myParser.py',307),
  ('if_condition -> RO exp RC','if_condition',3,'p_if_condition','myParser.py',321),
  ('if_condition -> RO error RC','if_condition',3,'p_if_condition_error','myParser.py',329),
  ('if_condition -> error exp RC','if_condition',3,'p_if_condition_error','myParser.py',330),
  ('if_condition -> RO exp error','if_condition',3,'p_if_condition_error','myParser.py',331),
  ('nt0_if -> empty','nt0_if',1,'p_nt0_if','myParser.py',346),
  ('nt1_if -> empty','nt1_if',1,'p_nt1_if','myParser.py',355),
  ('while -> WHILE while_condition nt0_while stmt','while',4,'p_while','myParser.py',366),
  ('while_condition -> RO exp RC','while_condition',3,'p_while_condition','myParser.py',377),
  ('while_condition -> RO error RC','while_condition',3,'p_while_condition_error','myParser.py',385),
  ('while_condition -> error exp RC','while_condition',3,'p_while_condition_error','myParser.py',386),
  ('while_condition -> RO exp error','while_condition',3,'p_while_condition_error','myParser.py',387),
  ('nt0_while -> empty','nt0_while',1,'p_nt0_while','myParser.py',402),
  ('exp_int -> INT','exp_int',1,'p_exp_int','myParser.py',415),
  ('exp_int -> MINUS INT','exp_int',2,'p_exp_int','myParser.py',416),
  ('exp_double -> DOUBLE','exp_double',1,'p_exp_double','myParser.py',427),
  ('exp_double -> MINUS DOUBLE','exp_double',2,'p_exp_double','myParser.py',428),
  ('exp -> exp AND exp','exp',3,'p_exp','myParser.py',440),
  ('exp -> exp OR exp','exp',3,'p_exp','myParser.py',441),
  ('exp -> NOT exp','exp',2,'p_exp','myParser.py',442),
  ('exp -> exp EQ EQ exp','exp',4,'p_exp','myParser.py',443),
  ('exp -> exp MIN exp','exp',3,'p_exp','myParser.py',444),
  ('exp -> exp MAJ exp','exp',3,'p_exp','myParser.py',445),
  ('exp -> exp MAJ_EQ exp','exp',3,'p_exp','myParser.py',446),
  ('exp -> exp EQ_MAJ exp','exp',3,'p_exp','myParser.py',447),
  ('exp -> exp MIN_EQ exp','exp',3,'p_exp','myParser.py',448),
  ('exp -> exp EQ_MIN exp','exp',3,'p_exp','myParser.py',449),
  ('exp -> exp PLUS exp','exp',3,'p_exp','myParser.py',450),
  ('exp -> exp MINUS exp','exp',3,'p_exp','myParser.py',451),
  ('exp -> exp STAR exp','exp',3,'p_exp','myParser.py',452),
  ('exp -> exp DIV exp','exp',3,'p_exp','myParser.py',453),
  ('exp -> RO exp RC','exp',3,'p_exp','myParser.py',454),
  ('exp -> exp_double','exp',1,'p_exp','myParser.py',455),
  ('exp -> exp_int','exp',1,'p_exp','myParser.py',456),
  ('exp -> id','exp',1,'p_exp','myParser.py',457),
  ('exp -> RO error RC','exp',3,'p_exp_error','myParser.py',513),
  ('id -> ID','id',1,'p_id','myParser.py',520),
  ('id -> ID SO INT SC','id',4,'p_id','myParser.py',521),
  ('id -> ID SO ID SC','id',4,'p_id','myParser.py',522),
  ('empty -> <empty>','empty',0,'p_empty','myParser.py',540),
]
