...
_lr_signature = 'leftORleftANDleftNOTleftMINMAJMIN_EQEQ_MINMAJ_EQEQ_MAJEQleftPLUSMINUSleftSTARDIVleftUMINUSnonassocIFXnonassocELSEAND BC BO CM DIV DOUBLE DOUBLE_TYPE ELSE EQ EQ_MAJ EQ_MIN ERROR ID IF INT INT_TYPE MAJ MAJ_EQ MIN MINUS MIN_EQ NOT OR PLUS PRINT RC RO S SC SO STAR WHILE comm nl ws\n        prog : decl_list stmt_list\n        \n        decl_list : decl_list decl \n                    | \n        \n        decl : type var_list S\n        \n        decl : type error S\n        \n        type : INT_TYPE \n                | DOUBLE_TYPE\n        \n        var_list : var \n                    | var_list CM var\n        \n        var : ID array\n        \n        array : empty\n                | array SO INT SC\n        \n        stmt_list : stmt_list stmt \n                    | stmt \n        \n        stmt_list : error stmt \n        \n        stmt : if \n                | while \n                | assignment \n                | print \n                | BO stmt_list BC\n        \n        assignment : id S \n                        | id EQ exp S \n        \n        stmt :  BO stmt_list error BC \n                | BO error BC \n                | error S \n        \n        assignment : id EQ error S \n                        | error EQ exp S \n        \n        print : PRINT id S\n        \n        if  : IF if_condition stmt %prec IFX\n                | IF if_condition stmt ELSE stmt\n        \n        if_condition : RO exp RC\n        \n        while : WHILE while_condition stmt\n        \n        while_condition : RO exp RC\n        \n        exp :   exp AND exp\n                | exp OR exp\n                | NOT exp\n                | exp EQ EQ exp\n                | exp MIN exp\n                | exp MAJ exp\n                | exp MAJ_EQ exp\n                | exp EQ_MAJ exp\n                | exp MIN_EQ exp\n                | exp EQ_MIN exp\n                | exp PLUS exp\n                | exp MINUS exp\n                | exp STAR exp\n                | exp DIV exp\n                | RO exp RC\n                | id\n                | INT\n                | DOUBLE\n                | MINUS INT %prec UMINUS\n                | MINUS DOUBLE %prec UMINUS\n        \n        id : ID\n            | ID SO INT SC\n            | ID SO ID SC\n        \n        empty :\n        '
...
_lr_productions = [
  ...
  ('prog -> decl_list stmt_list','prog',2,'p_prog','parser.py',41),
  ('decl_list -> decl_list decl','decl_list',2,'p_decl_list','parser.py',50),
  ('decl_list -> <empty>','decl_list',0,'p_decl_list','parser.py',51),
  ('decl -> type var_list S','decl',3,'p_decl','parser.py',56),
  ('decl -> type error S','decl',3,'p_decl_error','parser.py',62),
  ('type -> INT_TYPE','type',1,'p_type','parser.py',68),
  ('type -> DOUBLE_TYPE','type',1,'p_type','parser.py',69),
  ('var_list -> var','var_list',1,'p_var_list','parser.py',74),
  ('var_list -> var_list CM var','var_list',3,'p_var_list','parser.py',75),
  ('var -> ID array','var',2,'p_var','parser.py',80),
  ('array -> empty','array',1,'p_array','parser.py',85),
  ('array -> array SO INT SC','array',4,'p_array','parser.py',86),
  ('stmt_list -> stmt_list stmt','stmt_list',2,'p_stmt_list','parser.py',93),
  ('stmt_list -> stmt','stmt_list',1,'p_stmt_list','parser.py',94),
  ('stmt_list -> error stmt','stmt_list',2,'p_stmt_list_error','parser.py',100),
  ('stmt -> if','stmt',1,'p_stmt','parser.py',106),
  ('stmt -> while','stmt',1,'p_stmt','parser.py',107),
  ('stmt -> assignment','stmt',1,'p_stmt','parser.py',108),
  ('stmt -> print','stmt',1,'p_stmt','parser.py',109),
  ('stmt -> BO stmt_list BC','stmt',3,'p_stmt','parser.py',110),
  ('assignment -> id S','assignment',2,'p_assignment','parser.py',116),
  ('assignment -> id EQ exp S','assignment',4,'p_assignment','parser.py',117),
  ('stmt -> BO stmt_list error BC','stmt',4,'p_stmt_error','parser.py',122),
  ('stmt -> BO error BC','stmt',3,'p_stmt_error','parser.py',123),
  ('stmt -> error S','stmt',2,'p_stmt_error','parser.py',124),
  ('assignment -> id EQ error S','assignment',4,'p_assignment_error','parser.py',138),
  ('assignment -> error EQ exp S','assignment',4,'p_assignment_error','parser.py',139),
  ('print -> PRINT id S','print',3,'p_print','parser.py',162),
  ('if -> IF if_condition stmt','if',3,'p_if','parser.py',168),
  ('if -> IF if_condition stmt ELSE stmt','if',5,'p_if','parser.py',169),
  ('if_condition -> RO exp RC','if_condition',3,'p_if_condition','parser.py',174),
  ('while -> WHILE while_condition stmt','while',3,'p_while','parser.py',180),
  ('while_condition -> RO exp RC','while_condition',3,'p_while_condition','parser.py',185),
  ('exp -> exp AND exp','exp',3,'p_exp','parser.py',191),
  ('exp -> exp OR exp','exp',3,'p_exp','parser.py',192),
  ('exp -> NOT exp','exp',2,'p_exp','parser.py',193),
  ('exp -> exp EQ EQ exp','exp',4,'p_exp','parser.py',194),
  ('exp -> exp MIN exp','exp',3,'p_exp','parser.py',195),
  ('exp -> exp MAJ exp','exp',3,'p_exp','parser.py',196),
  ('exp -> exp MAJ_EQ exp','exp',3,'p_exp','parser.py',197),
  ('exp -> exp EQ_MAJ exp','exp',3,'p_exp','parser.py',198),
  ('exp -> exp MIN_EQ exp','exp',3,'p_exp','parser.py',199),
  ('exp -> exp EQ_MIN exp','exp',3,'p_exp','parser.py',200),
  ('exp -> exp PLUS exp','exp',3,'p_exp','parser.py',201),
  ('exp -> exp MINUS exp','exp',3,'p_exp','parser.py',202),
  ('exp -> exp STAR exp','exp',3,'p_exp','parser.py',203),
  ('exp -> exp DIV exp','exp',3,'p_exp','parser.py',204),
  ('exp -> RO exp RC','exp',3,'p_exp','parser.py',205),
  ('exp -> id','exp',1,'p_exp','parser.py',206),
  ('exp -> INT','exp',1,'p_exp','parser.py',207),
  ('exp -> DOUBLE','exp',1,'p_exp','parser.py',208),
  ('exp -> MINUS INT','exp',2,'p_exp','parser.py',209),
  ('exp -> MINUS DOUBLE','exp',2,'p_exp','parser.py',210),
  ('id -> ID','id',1,'p_id','parser.py',215),
  ('id -> ID SO INT SC','id',4,'p_id','parser.py',216),
  ('id -> ID SO ID SC','id',4,'p_id','parser.py',217),
