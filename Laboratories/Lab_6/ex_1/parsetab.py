...
_lr_productions = [
  ...
  ('prog -> decl_list stmt_list','prog',2,'p_prog','myParser.py',45),
  ('decl_list -> decl_list decl','decl_list',2,'p_decl_list','myParser.py',54),
  ('decl_list -> <empty>','decl_list',0,'p_decl_list','myParser.py',55),
  ('decl -> type var_list S','decl',3,'p_decl','myParser.py',60),
  ('decl -> type error S','decl',3,'p_decl_error','myParser.py',66),
  ('type -> INT_TYPE','type',1,'p_type','myParser.py',72),
  ('type -> DOUBLE_TYPE','type',1,'p_type','myParser.py',73),
  ('m_copy -> empty','m_copy',1,'p_m_copy','myParser.py',83),
  ('var_list -> var','var_list',1,'p_var_list','myParser.py',91),
  ('var_list -> var_list CM m_copy var','var_list',4,'p_var_list','myParser.py',92),
  ('var -> ID array','var',2,'p_var','myParser.py',103),
  ('array -> empty','array',1,'p_array','myParser.py',111),
  ('array -> array SO INT SC','array',4,'p_array','myParser.py',112),
  ('stmt_list -> stmt_list stmt','stmt_list',2,'p_stmt_list','myParser.py',124),
  ('stmt_list -> stmt','stmt_list',1,'p_stmt_list','myParser.py',125),
  ('stmt_list -> error stmt','stmt_list',2,'p_stmt_list_error','myParser.py',131),
  ('stmt -> if','stmt',1,'p_stmt','myParser.py',137),
  ('stmt -> while','stmt',1,'p_stmt','myParser.py',138),
  ('stmt -> assignment','stmt',1,'p_stmt','myParser.py',139),
  ('stmt -> print','stmt',1,'p_stmt','myParser.py',140),
  ('stmt -> BO stmt_list BC','stmt',3,'p_stmt','myParser.py',141),
  ('assignment -> id S','assignment',2,'p_assignment','myParser.py',147),
  ('assignment -> id EQ exp S','assignment',4,'p_assignment','myParser.py',148),
  ('stmt -> BO stmt_list error BC','stmt',4,'p_stmt_error','myParser.py',160),
  ('stmt -> BO error BC','stmt',3,'p_stmt_error','myParser.py',161),
  ('stmt -> error S','stmt',2,'p_stmt_error','myParser.py',162),
  ('assignment -> id EQ error S','assignment',4,'p_assignment_error','myParser.py',176),
  ('assignment -> error EQ exp S','assignment',4,'p_assignment_error','myParser.py',177),
  ('print -> PRINT id S','print',3,'p_print','myParser.py',199),
  ('print -> PRINT error S','print',3,'p_print_error','myParser.py',207),
  ('if -> IF if_condition nt0_if stmt ELSE nt1_if stmt','if',7,'p_if','myParser.py',215),
  ('if -> IF if_condition nt0_if stmt','if',4,'p_if','myParser.py',216),
  ('if -> IF if_condition nt0_if stmt error nt1_if stmt','if',7,'p_if','myParser.py',217),
  ('if_condition -> RO exp RC','if_condition',3,'p_if_condition','myParser.py',231),
  ('if_condition -> RO error RC','if_condition',3,'p_if_condition_error','myParser.py',238),
  ('if_condition -> error exp RC','if_condition',3,'p_if_condition_error','myParser.py',239),
  ('if_condition -> RO exp error','if_condition',3,'p_if_condition_error','myParser.py',240),
  ('nt0_if -> empty','nt0_if',1,'p_nt0_if','myParser.py',255),
  ('nt1_if -> empty','nt1_if',1,'p_nt1_if','myParser.py',263),
  ('while -> WHILE while_condition nt0_while stmt','while',4,'p_while','myParser.py',273),
  ('while_condition -> RO exp RC','while_condition',3,'p_while_condition','myParser.py',283),
  ('while_condition -> RO error RC','while_condition',3,'p_while_condition_error','myParser.py',289),
  ('while_condition -> error exp RC','while_condition',3,'p_while_condition_error','myParser.py',290),
  ('while_condition -> RO exp error','while_condition',3,'p_while_condition_error','myParser.py',291),
  ('nt0_while -> empty','nt0_while',1,'p_nt0_while','myParser.py',306),
  ('exp -> exp AND exp','exp',3,'p_exp','myParser.py',317),
  ('exp -> exp OR exp','exp',3,'p_exp','myParser.py',318),
  ('exp -> NOT exp','exp',2,'p_exp','myParser.py',319),
  ('exp -> exp EQ EQ exp','exp',4,'p_exp','myParser.py',320),
  ('exp -> exp MIN exp','exp',3,'p_exp','myParser.py',321),
  ('exp -> exp MAJ exp','exp',3,'p_exp','myParser.py',322),
  ('exp -> exp MAJ_EQ exp','exp',3,'p_exp','myParser.py',323),
  ('exp -> exp EQ_MAJ exp','exp',3,'p_exp','myParser.py',324),
  ('exp -> exp MIN_EQ exp','exp',3,'p_exp','myParser.py',325),
  ('exp -> exp EQ_MIN exp','exp',3,'p_exp','myParser.py',326),
  ('exp -> exp PLUS exp','exp',3,'p_exp','myParser.py',327),
  ('exp -> exp MINUS exp','exp',3,'p_exp','myParser.py',328),
  ('exp -> exp STAR exp','exp',3,'p_exp','myParser.py',329),
  ('exp -> exp DIV exp','exp',3,'p_exp','myParser.py',330),
  ('exp -> RO exp RC','exp',3,'p_exp','myParser.py',331),
  ('exp -> id','exp',1,'p_exp','myParser.py',332),
  ('exp -> INT','exp',1,'p_exp','myParser.py',333),
  ('exp -> DOUBLE','exp',1,'p_exp','myParser.py',334),
  ('exp -> MINUS INT','exp',2,'p_exp','myParser.py',335),
  ('exp -> MINUS DOUBLE','exp',2,'p_exp','myParser.py',336),
  ('exp -> RO error RC','exp',3,'p_exp_error','myParser.py',380),
  ('id -> ID','id',1,'p_id','myParser.py',387),
  ('id -> ID SO INT SC','id',4,'p_id','myParser.py',388),
  ('id -> ID SO ID SC','id',4,'p_id','myParser.py',389),
  ('empty -> <empty>','empty',0,'p_empty','myParser.py',405),
]
