...
_lr_productions = [
  ...
  ('prog -> decl_list stmt_list','prog',2,'p_prog','parser.py',39),
  ('decl_list -> decl_list decl','decl_list',2,'p_decl_list','parser.py',48),
  ('decl_list -> <empty>','decl_list',0,'p_decl_list','parser.py',49),
  ('decl -> type var_list S','decl',3,'p_decl','parser.py',54),
  ('type -> INT_TYPE','type',1,'p_type','parser.py',59),
  ('type -> DOUBLE_TYPE','type',1,'p_type','parser.py',60),
  ('var_list -> var','var_list',1,'p_var_list','parser.py',65),
  ('var_list -> var_list CM var','var_list',3,'p_var_list','parser.py',66),
  ('var -> ID array','var',2,'p_var','parser.py',71),
  ('array -> empty','array',1,'p_array','parser.py',76),
  ('array -> array SO INT SC','array',4,'p_array','parser.py',77),
  ('stmt_list -> stmt_list stmt','stmt_list',2,'p_stmt_list','parser.py',84),
  ('stmt_list -> stmt','stmt_list',1,'p_stmt_list','parser.py',85),
  ('stmt -> if','stmt',1,'p_stmt','parser.py',90),
  ('stmt -> while','stmt',1,'p_stmt','parser.py',91),
  ('stmt -> assignment','stmt',1,'p_stmt','parser.py',92),
  ('stmt -> print','stmt',1,'p_stmt','parser.py',93),
  ('stmt -> BO stmt_list BC','stmt',3,'p_stmt','parser.py',94),
  ('assignment -> id S','assignment',2,'p_assignment','parser.py',100),
  ('assignment -> id EQ exp S','assignment',4,'p_assignment','parser.py',101),
  ('print -> PRINT id S','print',3,'p_print','parser.py',107),
  ('if -> IF if_condition stmt','if',3,'p_if','parser.py',113),
  ('if -> IF if_condition stmt ELSE stmt','if',5,'p_if','parser.py',114),
  ('if_condition -> RO exp RC','if_condition',3,'p_if_condition','parser.py',119),
  ('while -> WHILE while_condition stmt','while',3,'p_while','parser.py',125),
  ('while_condition -> RO exp RC','while_condition',3,'p_while_condition','parser.py',130),
  ('exp -> exp AND exp','exp',3,'p_exp','parser.py',136),
  ('exp -> exp OR exp','exp',3,'p_exp','parser.py',137),
  ('exp -> NOT exp','exp',2,'p_exp','parser.py',138),
  ('exp -> exp EQ EQ exp','exp',4,'p_exp','parser.py',139),
  ('exp -> exp MIN exp','exp',3,'p_exp','parser.py',140),
  ('exp -> exp MAJ exp','exp',3,'p_exp','parser.py',141),
  ('exp -> exp MAJ_EQ exp','exp',3,'p_exp','parser.py',142),
  ('exp -> exp EQ_MAJ exp','exp',3,'p_exp','parser.py',143),
  ('exp -> exp MIN_EQ exp','exp',3,'p_exp','parser.py',144),
  ('exp -> exp EQ_MIN exp','exp',3,'p_exp','parser.py',145),
  ('exp -> exp PLUS exp','exp',3,'p_exp','parser.py',146),
  ('exp -> exp MINUS exp','exp',3,'p_exp','parser.py',147),
  ('exp -> exp STAR exp','exp',3,'p_exp','parser.py',148),
  ('exp -> exp DIV exp','exp',3,'p_exp','parser.py',149),
  ('exp -> RO exp RC','exp',3,'p_exp','parser.py',150),
  ('exp -> id','exp',1,'p_exp','parser.py',151),
  ('exp -> INT','exp',1,'p_exp','parser.py',152),
  ('exp -> DOUBLE','exp',1,'p_exp','parser.py',153),
  ('exp -> MINUS INT','exp',2,'p_exp','parser.py',154),
  ('exp -> MINUS DOUBLE','exp',2,'p_exp','parser.py',155),
  ('id -> ID','id',1,'p_id','parser.py',160),
  ('id -> ID SO INT SC','id',4,'p_id','parser.py',161),
  ('id -> ID SO ID SC','id',4,'p_id','parser.py',162),
