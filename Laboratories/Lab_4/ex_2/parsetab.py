...
_lr_productions = [
  ...
  ('prog -> decl_list stmt_list','prog',2,'p_prog','myParser.py',40),
  ('decl_list -> decl_list decl','decl_list',2,'p_decl_list','myParser.py',49),
  ('decl_list -> <empty>','decl_list',0,'p_decl_list','myParser.py',50),
  ('decl -> type var_list S','decl',3,'p_decl','myParser.py',55),
  ('decl -> type error S','decl',3,'p_decl_error','myParser.py',61),
  ('type -> INT_TYPE','type',1,'p_type','myParser.py',67),
  ('type -> DOUBLE_TYPE','type',1,'p_type','myParser.py',68),
  ('var_list -> var','var_list',1,'p_var_list','myParser.py',73),
  ('var_list -> var_list CM var','var_list',3,'p_var_list','myParser.py',74),
  ('var -> ID array','var',2,'p_var','myParser.py',79),
  ('array -> empty','array',1,'p_array','myParser.py',84),
  ('array -> array SO INT SC','array',4,'p_array','myParser.py',85),
  ('stmt_list -> stmt_list stmt','stmt_list',2,'p_stmt_list','myParser.py',92),
  ('stmt_list -> stmt','stmt_list',1,'p_stmt_list','myParser.py',93),
  ('stmt_list -> error stmt','stmt_list',2,'p_stmt_list_error','myParser.py',99),
  ('stmt -> if','stmt',1,'p_stmt','myParser.py',105),
  ('stmt -> while','stmt',1,'p_stmt','myParser.py',106),
  ('stmt -> assignment','stmt',1,'p_stmt','myParser.py',107),
  ('stmt -> print','stmt',1,'p_stmt','myParser.py',108),
  ('stmt -> BO stmt_list BC','stmt',3,'p_stmt','myParser.py',109),
  ('assignment -> id S','assignment',2,'p_assignment','myParser.py',115),
  ('assignment -> id EQ exp S','assignment',4,'p_assignment','myParser.py',116),
  ('stmt -> BO stmt_list error BC','stmt',4,'p_stmt_error','myParser.py',121),
  ('stmt -> BO error BC','stmt',3,'p_stmt_error','myParser.py',122),
  ('stmt -> error S','stmt',2,'p_stmt_error','myParser.py',123),
  ('assignment -> id EQ error S','assignment',4,'p_assignment_error','myParser.py',137),
  ('assignment -> error EQ exp S','assignment',4,'p_assignment_error','myParser.py',138),
  ('print -> PRINT id S','print',3,'p_print','myParser.py',160),
  ('if -> IF if_condition stmt','if',3,'p_if','myParser.py',166),
  ('if -> IF if_condition stmt ELSE stmt','if',5,'p_if','myParser.py',167),
  ('if_condition -> RO exp RC','if_condition',3,'p_if_condition','myParser.py',172),
  ('while -> WHILE while_condition stmt','while',3,'p_while','myParser.py',178),
  ('while_condition -> RO exp RC','while_condition',3,'p_while_condition','myParser.py',183),
  ('exp -> exp AND exp','exp',3,'p_exp','myParser.py',189),
  ('exp -> exp OR exp','exp',3,'p_exp','myParser.py',190),
  ('exp -> NOT exp','exp',2,'p_exp','myParser.py',191),
  ('exp -> exp EQ EQ exp','exp',4,'p_exp','myParser.py',192),
  ('exp -> exp MIN exp','exp',3,'p_exp','myParser.py',193),
  ('exp -> exp MAJ exp','exp',3,'p_exp','myParser.py',194),
  ('exp -> exp MAJ_EQ exp','exp',3,'p_exp','myParser.py',195),
  ('exp -> exp EQ_MAJ exp','exp',3,'p_exp','myParser.py',196),
  ('exp -> exp MIN_EQ exp','exp',3,'p_exp','myParser.py',197),
  ('exp -> exp EQ_MIN exp','exp',3,'p_exp','myParser.py',198),
  ('exp -> exp PLUS exp','exp',3,'p_exp','myParser.py',199),
  ('exp -> exp MINUS exp','exp',3,'p_exp','myParser.py',200),
  ('exp -> exp STAR exp','exp',3,'p_exp','myParser.py',201),
  ('exp -> exp DIV exp','exp',3,'p_exp','myParser.py',202),
  ('exp -> RO exp RC','exp',3,'p_exp','myParser.py',203),
  ('exp -> id','exp',1,'p_exp','myParser.py',204),
  ('exp -> INT','exp',1,'p_exp','myParser.py',205),
  ('exp -> DOUBLE','exp',1,'p_exp','myParser.py',206),
  ('exp -> MINUS INT','exp',2,'p_exp','myParser.py',207),
  ('exp -> MINUS DOUBLE','exp',2,'p_exp','myParser.py',208),
  ('id -> ID','id',1,'p_id','myParser.py',213),
  ('id -> ID SO INT SC','id',4,'p_id','myParser.py',214),
  ('id -> ID SO ID SC','id',4,'p_id','myParser.py',215),
  ('empty -> <empty>','empty',0,'p_empty','myParser.py',225),
]
